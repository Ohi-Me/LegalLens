...
import { useState } from "react"
// Ensure you have run: npm install jspdf
import ExecutiveSummary from "./components/ExecutiveSummary"
import StatCard from "./components/StatCard"
import SectionTitle from "./components/SectionTitle"
import RiskDistribution from "./components/RiskDistribution"
...
export default function Home() {
  ...
  const downloadReport = async () => {
    ...
    doc.setFontSize(18)
    doc.text("LegalLens Analysis Report", 14, y)
    y += 10
    ...
    // Fixed: Logic now correctly inside the function
    result.clauses.forEach((c: any, i: number) => {
      if (y > 270) {
        doc.addPage()
        y = 14
      ...
      const reasonsText = doc.splitTextToSize(`Reasons: ${c.reasons.join(", ")}`, 170)
      doc.text(reasonsText, 16, y)
      y += (reasonsText.length * 5)

      if (c.suggestion) {
        const suggestionText = doc.splitTextToSize(`Suggestion: ${c.suggestion}`, 170)
        doc.text(suggestionText, 16, y)
        y += (suggestionText.length * 5)
      }

      const bodyText = doc.splitTextToSize(c.text.slice(0, 300) + "...", 170)
      doc.text(bodyText, 16, y)
      y += (bodyText.length * 5) + 10
    })
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-6 py-12">
      <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl p-10">
        <h1 className="text-4xl font-bold text-slate-900 mb-2 tracking-tight">LegalLens</h1>
        <p className="text-slate-600 mb-10">Upload a legal contract PDF for AI risk analysis</p>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <button
            onClick={analyzeContract}
            disabled={loading || !file}
            className="px-8 py-2 rounded-lg bg-slate-900 text-white font-semibold hover:bg-slate-800 disabled:opacity-50"
          >
            {loading ? "Analyzing…" : "Analyze"}
          </button>
        </div>
        ...
        {error && <div className="mb-6 text-red-600 font-medium">{error}</div>}
        {loading && <div className="mb-6 text-slate-600 italic">First analysis may take ~30–60 seconds.</div>}

        {!result && !loading && (
          <div className="text-center text-slate-400 mt-24">Upload a PDF contract to begin analysis</div>
        )}

        {result && (
          <div id="results" className="mt-16 border-t pt-12 border-slate-200">
            <ExecutiveSummary overallRisk={result.overall_risk} missingCount={result.missing_clauses.length} />
            ...
            {result.missing_clauses.length > 0 ? (
              <div className="mb-12 flex flex-wrap gap-3">
                {result.missing_clauses.map((c: string, i: number) => (
                  <span key={i} className="px-4 py-1 rounded-full bg-red-100 text-red-700 text-sm font-semibold">
                    {c}
                  </span>
                ))}
              </div>
            ) : (
              <div className="mb-12 text-green-700 font-medium">✅ No critical clauses missing</div>
            )}
