...
import { useState } from "react"
// Note: Ensure you have run 'npm install jspdf' in your terminal
import ExecutiveSummary from "./components/ExecutiveSummary"
import StatCard from "./components/StatCard"
import SectionTitle from "./components/SectionTitle"
import RiskDistribution from "./components/RiskDistribution"
...
export default function Home() {
  ...
  const downloadReport = async () => {
    ...
    // Dynamic import to prevent SSR issues with jspdf
    const { default: jsPDF } = await import("jspdf")
    const doc = new jsPDF()
    let y = 14

    doc.setFontSize(18)
    doc.text("LegalLens Risk Report", 14, y)
    y += 10
    ...
    result.clauses.forEach((c: any, i: number) => {
      ...
      if (c.suggestion) {
        const suggestionLines = doc.splitTextToSize(`Suggestion: ${c.suggestion}`, 170)
        doc.text(suggestionLines, 16, y)
        y += (suggestionLines.length * 5)
      }

      const textLines = doc.splitTextToSize(c.text.slice(0, 300) + "...", 170)
